...
bool TopologyMap::ReadLaunchParams(ros::NodeHandle & nodeHandle) {
	...
	m_bMapFileFlag = m_sFileHead.empty();

	if(!m_bMapFileFlag) {
		if(m_sFileHead.back() != '/') m_sFileHead += "/";
		std::stringstream sOutputCommand;
		sOutputCommand << "rm -rf " << m_sFileHead;
...
void TopologyMap::HandleTrajectory(const nav_msgs::Odometry & oTrajectory) {
	...
	if (!(m_iTrajFrameNum % m_iOdomSampingNum)) {
		...
		if(!m_bAnchorGoalFlag){
			...
			std::cout<<"move in global way ( check the robot is near the target node)"<<std::endl;
        ...
        if(fTouchNodeGoal){
			...
			//get new nodes
			// std::cout<<"get new nodes"<<std::endl;
			...
			std::cout<<"clear old data of last trip"<<std::endl;
			...
			std::cout<<"compute astar path for current target point"<<vUnvisitedNodes.size()<<std::endl;
...
void TopologyMap::PublishGoalOdom(pcl::PointXYZ & oGoalPoint){
        ...
        oCurrGoalOdom.header.frame_id = "base_link_odom" ;
