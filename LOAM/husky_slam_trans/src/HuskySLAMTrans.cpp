...
SLAMTrans::SLAMTrans(ros::NodeHandle & node, 
    ...
    m_oHighOdomSuber = node.subscribe("/odometry/filtered", 1, &SLAMTrans::HandleOdom, this);
    ...
    //publish odom topic of lidar 
    m_oHighOdomPub = node.advertise<nav_msgs::Odometry>("odom_lidar", 1);
...
void SLAMTrans::HandlePointClouds(const sensor_msgs::PointCloud2 & vLaserData)
{
  ...
  if(!(m_iFrames%m_iSampleNum)){
    ...
    // std::cout<<" vTransposedData.fields.size: "<<  vTransposedData.fields.size()<<std::endl;
    // float x,y,z,intensity;
    // int ring;
    // int step = 22;
    // for(int i = 0; i < vTransposedData.data.size(); i +=step) {
    //     std::memcpy(&x,&vTransposedData.data[step*i],4);
    //     std::memcpy(&y,&vTransposedData.data[step*i+4],4);
    //     std::memcpy(&z,&vTransposedData.data[step*i+8],4);
    //     std::memcpy(&intensity,&vTransposedData.data[step*i+12],4);
    //     std::memcpy(&ring,&vTransposedData.data[step*i+16],2);
    //     std::cout<<"x: "<<x<<"  y: "<<y<<"  z: "<<z<<"  intensity: "<<intensity<<"  ring: "<<ring<<std::endl;
    // } std:: cout << std::endl;


    // vTransposedData.fields.at(3).name = "ring";
    // vTransposedData.fields.at(4).name = "intensity";
...
void SLAMTrans::HandleOdom(const nav_msgs::Odometry & oTrajectory)
{
