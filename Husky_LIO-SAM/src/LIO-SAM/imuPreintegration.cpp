...
public:
    ...
    tf::TransformBroadcaster tfBaseLink2BaseLink_odomFrame;
    tf::Transform baselink_to_baselinkodomFrame = tf::Transform(tf::createQuaternionFromRPY(0, 0, 0), tf::Vector3(0, 0, 0));
    ...
    TransformFusion()
    {
        ...
        if(lidarFrame != baselinkFrame)
        {
            try
            {
                ...
                tfBaseLink2BaseLink_odomFrame.sendTransform(tf::StampedTransform(lidar2Baselink, ros::Time(0), odometryFrame,"base_link_odom"));    

                ///*Debug
                #ifdef DEBUG
                    std::cout<<"\033[31mLIO-SAM DEBUG \033[1m\033[34mlidar2Baselink:";
                    std::cout << "  Translation: " << lidar2Baselink.getOrigin().getX() << ", " << lidar2Baselink.getOrigin().getY() << ", " << lidar2Baselink.getOrigin().getZ();
                    std::cout << "  Rotation: " << lidar2Baselink.getRotation().getX() << ", " << lidar2Baselink.getRotation().getY() << ", " << lidar2Baselink.getRotation().getZ() << ", " << lidar2Baselink.getRotation().getW();
    ...
    */
    void lidarOdometryHandler(const nav_msgs::Odometry::ConstPtr& odomMsg)
    {
        ...
        //Debug lidar odometry
        // std::cout<<"\033[1m\033[31m  imuPreintergration lidar odometry : "<<std::endl;
        // std::cout<<lidarOdomAffine.matrix()<< "\033[0m"<<std::endl;
   ...
   // ??? 坐标系变换是怎么做的?
    void imuOdometryHandler(const nav_msgs::Odometry::ConstPtr& odomMsg)
    {
        ...
        // 当前时刻imu里程计位姿 = 最近的一帧激光里程计时刻对应的imu位姿 * imu里程计增量位姿变换
        ...
        if(incremental - 0.01 < 0 )
        {
            tCur = lastIMUodomCur;
            /*Debug
            std::cout<<"\033[1m\033[31m"<<"delta_position(<0.01):    "<< incremental - 0.01<<std::endl;
            std::cout<< "\033[0m"<<std::endl;//*/
        } 
        else
        {
        ...
        if(lidarFrame != baselinkFrame) 
        {
            ...
            //载体定位系
            baselink_to_baselinkodomFrame = lidar2Baselink;
        }
        tf::StampedTransform odom_2_baselink = tf::StampedTransform(tCur, odomMsg->header.stamp, odometryFrame, baselinkFrame);
        tfOdom2BaseLink.sendTransform(odom_2_baselink);
        // 发布载体的odom系用于后续开发，事实上与载体系相同
        // 考虑到后续开发，坐标系统一先建立，有没有用调试完在看
        tfBaseLink2BaseLink_odomFrame.sendTransform(tf::StampedTransform(baselink_to_baselinkodomFrame, odomMsg->header.stamp, odometryFrame,"base_link_odom"));     
