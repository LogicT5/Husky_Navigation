...
public:
    ...
    vector<gtsam::noiseModel::Diagonal::shared_ptr> loopNoiseQueueFE;
    ...
    mapOptimization()
    {
        ...
        if(featureExtracted)
            subCloud = nh.subscribe<husky_lio_sam::cloud_info>("husky_lio_sam/feature/cloud_info", 1, &mapOptimization::laserCloudInfoHandler, this, ros::TransportHints().tcpNoDelay());
        else
            subCloud = nh.subscribe<husky_lio_sam::cloud_info>("husky_lio_sam/deskew/cloud_info", 1, &mapOptimization::laserCloudInfoHandler, this, ros::TransportHints().tcpNoDelay());
         // 订阅GPS里程计
    ...
    */
    void performLoopClosure()
    {
        ...
        *copy_cloudKeyPoses6D = *cloudKeyPoses6D;
        ...
        loopNoiseQueueFE.push_back(constraintNoise);
...
void loopFindNearKeyframes(pcl::PointCloud<PointType>::Ptr& nearKeyframes, const int& key, const int& searchNum, const int& loop_index)
    ...
    void extractCloud(pcl::PointCloud<PointType>::Ptr cloudToExtract)
    {
        ...
        for (int i = 0; i < (int)cloudToExtract->size(); ++i)
        {
            ...
            if (laserCloudMapContainer.find(thisKeyInd) != laserCloudMapContainer.end()) 
            {
                ...
                if(featureExtracted){
                    laserCloudCornerTemp = *transformPointCloud(cornerCloudKeyFrames[thisKeyInd],  &cloudKeyPoses6D->points[thisKeyInd]);
    ...
    */
    ...
    /**
     * 当前激光帧角点寻找局部map匹配点
     * 1、更新当前帧位姿，将当前帧角点坐标变换到map系下，
     *       在局部map中查找5个最近点，距离小于1m，且5个点构成直线
    ...
    */
    bool saveFrame()
    {
        ...
        if (sensor == SensorType::LIVOX && featureExtracted)
        {
    ...
    */
    void addLoopFactor()
    {
        ...
        for (int i = 0; i < (int)loopIndexQueue.size(); ++i)
        {
            ...
            gtsam::noiseModel::Diagonal::shared_ptr noiseBetweenFE = loopNoiseQueueFE[i];
            auto noiseBetween = loopNoiseQueue[i];
            if (featureExtracted)
                gtSAMgraph.add(BetweenFactor<Pose3>(indexFrom, indexTo, poseBetween, noiseBetweenFE));
            else
