...
void saveCloud(const sensor_msgs::PointCloud2& cloud_msg)
{
    ...
    int n = now_cloud.size();
    for(int i = 0; i < n; ++i) {
        pcl::PointXYZ point = now_cloud[i];
        if(point.y > 0 && abs(point.x) < point.y && point.x*point.x + point.y*point.y + point.z*point.z < 100) {
            now_cloud[i--] = now_cloud[--n];
            now_cloud[n] = point;
        }
    }
    now_cloud.erase(now_cloud.begin()+n, now_cloud.end());
