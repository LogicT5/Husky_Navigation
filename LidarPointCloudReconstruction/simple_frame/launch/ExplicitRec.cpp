...
void ExplicitRec::RemovePseudoFacesAndComputeCenterPoint(const pcl::PointCloud<pcl::PointXYZI> & vSectorPoints, pcl::PointCloud<pcl::PointXYZI> & vCenterPoints, const std::vector<pcl::Vertices> & vFaces, const Eigen::MatrixXf & oMatNormal,
	...
	vCenterPoints.clear();
	vCenterPoints.reserve(vFaces.size());
	//Eigen::Vector3f oHorPlanN(0.0f, 0.0f, 1.0f);
	//float fVerticalThr = 0.17365;
	// for each face
	for (int i = 0; i != vFaces.size(); ++i){
		...
		for (int j = 0; j != vFaces[i].vertices.size(); ++j){
			oCenterPoint.x = oCenterPoint.x + vSectorPoints.points[vFaces[i].vertices[j]].x;
			oCenterPoint.y = oCenterPoint.y + vSectorPoints.points[vFaces[i].vertices[j]].y;
			oCenterPoint.z = oCenterPoint.z + vSectorPoints.points[vFaces[i].vertices[j]].z;
			
			//判断这个面是否要剔除
			int index_a = j;
			int index_b = j;
			if (j == vFaces[i].vertices.size() - 1)
				index_b = 0;
			else
				index_b = j + 1;
		...
		//take the average
		float fNum = float(vFaces[i].vertices.size());
