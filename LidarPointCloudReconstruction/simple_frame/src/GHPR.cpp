...
std::vector<pcl::Vertices> GHPR::ConstructSurfaceIdxFiltered(const int line_min, const int line_max, const bool debug){
	...
	// if(debug) std::cout<<"m_vHullPolygonIdxs.size "<<m_vHullPolygonIdxs.size()<<std::endl;
	// if(debug) std::cout<<"m_pTransCloud.size "<<m_pTransCloud->points.size()<<std::endl;
	...
	for (int i = 0; i != m_vHullPolygonIdxs.size(); ++i){
		...
		// if(debug) std::cout<<"m_vHullPolygonIdxs["<<i<<"].vertices.size  "<<m_vHullPolygonIdxs[i].vertices.size()<<std::endl;
		...
		for (int j = 0; j != m_vHullPolygonIdxs[i].vertices.size(); ++j){
			...
			int iOneLocalIdx = m_vHullPolygonIdxs[i].vertices[j];
			int iOneWorldIdx = m_vHullInInputIdx[iOneLocalIdx];
			// if(debug) std::cout<<"iOneWorldIdx "<<iOneWorldIdx<<std::endl;
			// if(debug) std::cout<<"m_iViewWorldIdx "<<m_iViewWorldIdx<<std::endl;
		...
		if(debug) std::cout<<"bAllZeroLine "<<bAllZeroLine<<std::endl;
		if(debug) std::cout<<"bAllMaxLine "<<bAllMaxLine<<std::endl;
		...
		if(debug) std::cout<<"bAllZeroLine "<<bAllZeroLine<<std::endl;
		if(debug) std::cout<<"bAllMaxLine "<<bAllMaxLine<<std::endl;
		if(debug) std::cout<<(bNonViewPoint && !bAllZeroLine && !bAllMaxLine)<<std::endl;
